...
void main() {
    ...
    int newSize;
    if (((int)(ceil((log(size) / log(2)))) == (int)(floor((log(size) / log(2)))))) {
        newSize = size;
    }
    else {
        int power = (int)(ceil((log(size) / log(2))));
        newSize = (int)pow(2, power);

    }   

    int **newArrayA;
    newArrayA = malloc(newSize * sizeof(*newArrayA));
    for(int i = 0 ; i < newSize ; i++) {
        newArrayA[i] = malloc(newSize * sizeof(*newArrayA));
    }
    int **newArrayB;
    newArrayB= malloc(newSize * sizeof(*newArrayB));
    for(int i = 0 ; i < newSize ; i++) {
        newArrayB[i] = malloc(newSize * sizeof(*newArrayB));
    }
    ...
    int **arrayC = Multiply(newArrayA, newArrayB, newSize);
    printf("Array C is : \n");
    for (int i = 0 ; i < size ; i++) {
        printf("\t\t[ ");
        for(int j = 0 ; j < size ; j++) {
